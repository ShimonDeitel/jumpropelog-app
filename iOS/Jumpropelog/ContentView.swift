    ...
    private func row(_ entry: SessionEntry) -> some View {
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 4) {
                ...
                Text("Duration (min): \(entry.value1)  \u{00B7}  Skip Count: \(entry.value2)")
                    .font(.caption)
...
struct EntrySheet: View {
    ...
    @State private var option: String = JumpropelogOptions.all.first ?? ""
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Style") {
                    Picker("Style", selection: $option) {
                        ForEach(JumpropelogOptions.all, id: \.self) { t in
                            Text(t).tag(t)
                        }
                    }
                    .pickerStyle(.menu)
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        if var existing = entry {
                            ...
                            store.add(style: option, value1: Int(value1), value2: Int(value2), note: note)
                        }
                        dismiss()
                    }
